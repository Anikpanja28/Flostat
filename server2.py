...
OTHER_SERVERS = ['http://192.168.1.7:5001', 'http://192.168.1.7:5002']  # Replace with actual server IPs
...
def synchronize_with_other_servers():
    print("Synchronizing with other servers...")
    
    for server in OTHER_SERVERS:
        try:
            # Fetch the latest water levels from the other server
            response = requests.get(f'{server}/api/get_all_water_levels')
            if response.status_code == 200:
                water_levels = response.json()['water_levels']
            ...
            response = requests.get(f'{server}/api/get_all_valve_states')
            if response.status_code == 200:
                valve_states = response.json()['valve_states']
            ...
            print(f"Failed to synchronize with {server}: {e}")
    ...
    for server_url in OTHER_SERVERS:
        try:
            if data_type == 'water_level':
                requests.post(f'{server_url}/api/sync_water_level', json={'tank_id': tank_id, 'water_level': data})
            elif data_type == 'valve_control':
                requests.post(f'{server_url}/api/sync_valve_control', json={'tank_id': tank_id, 'command': data, 'server': server})
        except Exception as e:
...
if __name__ == "__main__":
    create_db()
    synchronize_with_other_servers()  # Synchronize with other servers before starting
